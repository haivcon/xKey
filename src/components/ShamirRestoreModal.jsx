...
export default function ShamirRestoreModal({ aesKey, onClose, onRestore }) {
  const [shares, setShares] = useState({});
  const [pagesByPart, setPagesByPart] = useState({});
  const [error, setError] = useState('');
  const [password, setPassword] = useState('');
  const [decrypting, setDecrypting] = useState(false);
  const [previewWallets, setPreviewWallets] = useState(null);
  ...
  useEffect(() => {
    ...
    const startScanner = async () => {
      try {
        ...
        const scanner = new Html5Qrcode('shamir-restore-reader', { formatsToSupport: [0] });
        ...
        if (!stopped) setError(typeof err === 'string' ? err : (err.message || t('qrScanner.cameraDenied')));
      }
    };
  ...
  const restore = async () => {
    ...
    try {
      ...
      const wallets = Array.isArray(backup.wallets) ? backup.wallets : [];
  ...
  const confirmRestore = async () => {
    ...
    try {
      ...
      setError(err?.message || t('shamir.corruptedShares'));
    } finally {
      setImporting(false);
    }
  };
