...
export default function HDWalletTreeVisualizer({ onSaveWallet, existingWallets = [] }) {
  const t = useT();
  ...
  const selectedNetwork = HD_NETWORKS.find(item => item.value === network) || HD_NETWORKS[0];
  ...
  const deriveWallets = useCallback(() => {
    ...
    try {
      ...
      const nextWallets = Array.from({ length: leafCount }, (_, index) => {
        const path = `m/44'/${selectedNetwork.coinType}'/${account}'/0/${index}`;
        const wallet = ethers.HDNodeWallet.fromPhrase(phrase, null, path);
        return {
