...
type MoveToFolderModalProps = {
  wallet: Wallet;
  folders: string[];
  onMove: (wallet: Wallet, targetFolder: string) => void;
  onClose: () => void;
};

export default function MoveToFolderModal({ wallet, folders, onMove, onClose }: MoveToFolderModalProps) {
  const t = useT();
  ...
  return (
    <div className="app-scaled-icons fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-surface-900 border border-surface-700 w-full max-w-sm rounded-2xl shadow-2xl p-6" onClick={(e: MouseEvent<HTMLDivElement>) => e.stopPropagation()}>
