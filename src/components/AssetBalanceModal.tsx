...
type AssetBalanceFilter = 'all' | 'positive' | 'zero' | 'changed';
type AutoSaveState = 'idle' | 'pending' | 'saving' | 'saved' | 'error';
type SavePayloadItem = { wallet: Wallet; balance: string };

type AssetBalanceModalProps = {
  wallets: Wallet[];
  assetUnit?: string;
  totalBalance: number;
  onClose: () => void;
  onSave: (payload: SavePayloadItem[], options?: { silent?: boolean }) => void | Promise<void>;
...
export default function AssetBalanceModal({ wallets, assetUnit, totalBalance, onClose, onSave, onUnitChange, t }: AssetBalanceModalProps) {
  const showConfirm = useConfirm();
  const { showToast } = useToast() || {};
  const [unit, setUnit] = useState(assetUnit || '$');
  const [balances, setBalances] = useState<BalanceMap>({});
  const [originalBalances, setOriginalBalances] = useState<BalanceMap>({});
  const [query, setQuery] = useState('');
  const [filter, setFilter] = useState<AssetBalanceFilter>('all');
  const [copiedAddressKey, setCopiedAddressKey] = useState('');
  const [autoSaveState, setAutoSaveState] = useState<AutoSaveState>('idle');
  const [savedAtByKey, setSavedAtByKey] = useState<SavedAtMap>({});
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const balanceInputRefs = useRef(new Map<string, HTMLInputElement>());
  ...
  const getWalletKey = useCallback((wallet: Wallet, index: number) => wallet._id || `${wallet.address || 'wallet'}-${index}`, []);
  ...
  const buildSavePayload = useCallback((balanceMap: BalanceMap = balances): SavePayloadItem[] => wallets.map((wallet, index) => ({
    wallet,
  ...
  const importCsv = async (file?: File) => {
    ...
    const addressToKey = new Map<string, string>(
      wallets
        .map((wallet, index): [string | undefined, string] => [wallet.address?.toLowerCase(), getWalletKey(wallet, index)])
  ...
  const copyAddress = async (wallet: Wallet, key: string) => {
  ...
  const getBalanceWarning = (key: string) => {
  ...
  const focusNextBalance = (visibleIndex: number) => {
      ...
      <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-surface-700 bg-surface-900 shadow-2xl">
        ...
        <div className="max-h-[70dvh] overflow-y-auto p-4">
          <div className="mb-4 rounded-2xl border border-surface-700 bg-surface-950/50 p-3">
            ...
            <input
              value={unit}
              onChange={(e: ChangeEvent<HTMLInputElement>) => handleUnitChange(e.target.value)}
              placeholder="$"
          ...
          <div className="mb-3 flex justify-center gap-1.5 overflow-x-auto pb-1 sm:justify-start">
            {(['all', 'positive', 'zero', 'changed'] as AssetBalanceFilter[]).map(key => (
              <button
                key={key}
                type="button"
                ...
                <div key={key} className={`grid grid-cols-1 gap-3 rounded-2xl border p-3 text-center sm:grid-cols-[2.5rem_minmax(0,1fr)_9rem] sm:items-center sm:text-left ${changed ? 'border-brand-500/50 bg-brand-500/10' : 'border-surface-700 bg-surface-800/45'}`}>
                  ...
                  <div className="flex min-w-0 items-center overflow-hidden rounded-xl border border-surface-700 bg-surface-950/80 focus-within:border-brand-500 sm:w-36">
                    ...
                    <input
                      ...
                      onChange={(e: ChangeEvent<HTMLInputElement>) => setBalances(prev => ({ ...prev, [key]: normalizeAmountInput(e.target.value) }))}
