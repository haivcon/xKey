...
export default function MarkdownRenderer({ text, className = '' }: MarkdownRendererProps) {
  ...
  const renderInline = (line: string, key: string): ReactNode[] => {
    ...
    while (remaining.length > 0) {
      ...
      if (match && match.index === 0) {
        parts.push(
          <code key={`${key}-${idx++}`} className="px-1 py-0.5 bg-surface-700 rounded text-brand-300 text-scale-xs font-mono">
            {match[1]}
          </code>
        );
        remaining = remaining.slice(match[0].length);
        ...
        <ul key={`ul-${elements.length}`} className="space-y-0.5 ml-3">
          {listItems.map((item, i) => (
            <li key={i} className="flex items-start gap-1.5">
              <span className="text-brand-400 mt-0.5 text-scale-3xs">●</span>
  ...
  return (
    <div className={`text-scale-xs text-surface-400 leading-relaxed space-y-1 ${className}`}>
      {elements}
    </div>
  );
}
