...
import { X, Wrench, ShieldCheck, Tags, Download, GitCompare, Search, Network, History, LockKeyhole, Merge, Sparkles, ClipboardCheck, RefreshCw } from 'lucide-react';
...
const parseWalletFile = async (file, aesKey, password) => {
...
export default function AdvancedToolsModal({
  ...
  const [activeTool, setActiveTool] = useState('check');
  ...
  const compareFiles = async () => {
    try {
      const result = await FilePicker.pickFiles({ multiple: true, readData: true, types: ['.csv', '.json', '.xkey', 'text/csv', 'application/json', 'application/octet-stream', '*/*'] });
...
function InputAction({ value, onChange, placeholder, action, actionText }) {
  return (
    <div className="flex gap-2">
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="min-w-0 flex-1 rounded-lg border border-surface-700 bg-surface-900 px-3 py-2 text-xs text-white outline-none focus:border-brand-500 sm:text-sm" />
