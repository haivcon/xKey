...
type ScrambledKeyboardMode = 'sensitive' | 'all';
type ManagedInputElement = HTMLInputElement | HTMLTextAreaElement;
type ScrambledKeyboardChangeEvent = { target: { value: string } };
type ScrambledKeyboardHistoryItem = {
  value: string;
  cursor: number;
};
type ScrambledKeyboardActiveState = {
  value: string;
  cursor: number;
  label: string;
  onChange?: (event: ScrambledKeyboardChangeEvent) => void;
  element: ManagedInputElement | null;
  masked: boolean;
  history: ScrambledKeyboardHistoryItem[];
};
type OpenKeyboardConfig = {
  value?: string;
  cursor?: number | null;
  label?: string | null;
  onChange?: (event: ScrambledKeyboardChangeEvent) => void;
...
type ScrambledKeyboardContextValue = {
  enabled: boolean;
  setEnabled: (next: boolean) => void;
  mode: ScrambledKeyboardMode;
  setMode: (next: string) => void;
  openKeyboard: (config: OpenKeyboardConfig) => void;
  openKeyboardForElement: (element: ManagedInputElement) => void;
  closeKeyboard: () => void;
};

const ScrambledKeyboardContext = createContext<ScrambledKeyboardContextValue | null>(null);
...
const setNativeValue = (element: ManagedInputElement, value: string) => {
...
const isEditableElement = (target: unknown): target is ManagedInputElement => {
...
const isSensitiveElement = (element: ManagedInputElement): boolean => {
...
export function useScrambledKeyboard(): ScrambledKeyboardContextValue {
...
export function ScrambledKeyboardProvider({ children }: { children: ReactNode }) {
  const [enabled, setEnabledState] = useState(false);
  const [mode, setModeState] = useState<ScrambledKeyboardMode>('sensitive');
  const [active, setActive] = useState<ScrambledKeyboardActiveState | null>(null);
  const [shift, setShift] = useState(false);
  const [keys, setKeys] = useState(() => shuffle([...LOWER, ...DIGITS, ...SYMBOLS]));
  const activeRef = useRef<ScrambledKeyboardActiveState | null>(null);
  const t = useT();
  ...
  const setMode = useCallback((next: string) => {
    const normalized: ScrambledKeyboardMode = next === 'all' ? 'all' : 'sensitive';
  ...
  const openKeyboard = useCallback((config: OpenKeyboardConfig) => {
    if (!enabled) return;
    const rawValue = String(config.value || config.element?.value || '');
    const cursor = typeof config.cursor === 'number' && Number.isInteger(config.cursor) ? config.cursor : rawValue.length;
    setShift(false);
    setKeys(shuffle([...LOWER, ...DIGITS, ...SYMBOLS]));
    setActive({
      value: rawValue,
  ...
  const openKeyboardForElement = useCallback((element: ManagedInputElement) => {
    if (!enabled || !isEditableElement(element)) return;
    const cursor = typeof element.selectionStart === 'number' ? element.selectionStart : element.value.length;
    openKeyboard({
      element,
      value: element.value,
      cursor,
  ...
  useEffect(() => {
    ...
    const handler = (event: PointerEvent) => {
      if (event.target instanceof Element && event.target.closest('[data-scrambled-keyboard="true"]')) return;
  ...
  const applyValue = useCallback((nextValue: string, nextCursor: number, pushHistory = true) => {
  ...
  const setCursorFromPreview = useCallback((event: ReactMouseEvent<HTMLButtonElement>) => {
    const current = activeRef.current;
    if (!current) return;
