...
const renderTone = (frequency, duration, volume) => {
  try {
...
const playTone = (frequency, duration = 0.035, volume = 0.035) => {
  ...
  try {
    ...
    if (audioContext.state === 'suspended') {
      audioContext.resume().then(() => {
        if (audioContext.state === 'running') renderTone(frequency, duration, volume);
      }).catch(() => {});
      return;
    }
...
const vibrate = (pattern, nativeFeedback) => {
