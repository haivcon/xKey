...
const getAuditSecret = async (): Promise<string> => {
...
const readRawEntries = async (): Promise<RawAuditEntry[]> => {
  const { value } = await Preferences.get({ key: AUDIT_LOG_KEY });
  if (!value) return [];
  try {
    const entries = JSON.parse(value);
    return Array.isArray(entries) ? entries as RawAuditEntry[] : [];
  } catch {
    return [];
  }
};

const decryptPayload = (cipher: string, secret: string): AuditPayload => {
...
export const appendAuditLog = async (type: string, details: AuditDetails = {}): Promise<boolean> => {
  try {
    ...
    const previousHash = entries[entries.length - 1]?.entryHash || 'GENESIS';
...
export const readAuditLog = async (): Promise<AuditLogReadResult> => {
  const secret = await getAuditSecret();
  const entries = await readRawEntries();
